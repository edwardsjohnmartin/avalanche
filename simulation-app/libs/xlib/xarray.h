...
namespace xlib {
	...
	public:
		...
		xarray(unsigned int xsize) {
			_x = new T[xsize];
			_ysize = 1;
			_xsize = xsize;
		}
		xarray(unsigned int xsize, unsigned int ysize) {
			_x = new T[xsize*ysize];
			_xsize = xsize;
			_ysize = ysize;
		}
		xarray(const T* arry, unsigned int xsize) {
			_x = new T[xsize];
			_xsize = xsize;
			_ysize = 1;
		...
		inline xarray operator =(const xarray<T> &xa) {
			delete[] _x;
			_x = new T[xa._xsize*xa._ysize];
			_xsize = xa._xsize;
			_ysize = xa._ysize;
		...
		inline xarray operator =(const std::vector<T> &v) {
			clear();
			resize(v.size());
		...
		friend bool operator ==(const xarray<T>& xa, const xarray<T>& xb) {
